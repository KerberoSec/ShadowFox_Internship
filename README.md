...
| # | Task | Tool(s) | Key Finding | Status |
|---|------|---------|-------------|--------|
| 1 | Open Port Discovery | Nmap 7.98 | Port 80 open with IIS 8.5; missing security headers | ✅ Completed |
| 2 | Directory Enumeration | Gobuster 3.8.2 + FFUF 2.1 | 14 directories found including sensitive system folders | ✅ Completed |
...
| `-Pn` | Skip host discovery; treat target as online regardless of ICMP response |
...
| HttpOnly flag missing on session cookie | 🟡 Medium | Session cookies accessible to JavaScript; exploitable via XSS |

---

## 📂 Task 2 — Directory Enumeration Using Gobuster & FFUF
...
| `/cgi-bin/` | 301 → 403 | 🔴 High | CGI directory confirmed — historically exploitable for RCE |
...
| `/t/` & `/T/` | 301 | 🟡 Medium | Ambiguous paths warrant further investigation |
...
> ⚠️ **URL encoding is NOT encryption.** The `%40` → `@` substitution is trivially reversible and provides zero security. The session cookie `ASPSESSIONIDCQDABSCC=GNPAOENAJCMDFIDFDCIDLFKO` was also visible in every captured request, enabling session hijacking.
...
|----|--------------|----------|------|---------------|
| V01 | Insecure HTTP Transmission (No HTTPS) | 🔴 Critical | 9.1 | Task 1 & 3 |
...
|----------|---------------|-----------|
| 🔴 Critical | Implement HTTPS with TLS 1.2+ and HSTS | V01, V02, V03 |
| 🟠 High | Upgrade IIS 8.5 → IIS 10.0 on Windows Server 2022 | V04 |
| 🟠 High | Set HttpOnly and Secure flags on all session cookies | V05 |
| 🟡 Medium | Disable HTTP TRACE method via web.config | V06 |
| 🟠 High | Restrict/remove sensitive directories (CGI, FrontPage) | V07, V08, V09 |
| 🟡 Medium | Implement Multi-Factor Authentication (TOTP/SMS) | V01, V02 |
| 🟡 Medium | Add login rate limiting and account lockout after 5 failures | V01, V02 |
| 🟢 Low | Sanitise robots.txt — remove sensitive path disclosures | V07 |
| 🟡 Medium | Conduct regular security assessments (annual pentest + monthly scans) | All |
...
| **FFUF** | 2.1.0 | Web fuzzing — cross-validation of directory enumeration |
...
```
📦 shadowfox-beginner-security-assessment
 ┣ 📄 README.md                                    ← You are here
 ┗ 📄 Arun_Kumar_ShadowFox_Beginner_Report_Final.pdf  ← Full detailed report
```
...
- ✅ Hands-on experience with the full recon → exploitation → reporting lifecycle
